...
:mySiPM(sipm)
{
  Dir = new G4UIdirectory("/sipm/");
	...
	translateSiPM= new G4UIcmdWith3VectorAndUnit("/sipm/translate",this);
	translateSiPM->SetGuidance("Translate SiPM by a vector");
	translateSiPM->SetUnitCategory("Lenght");
	translateSiPM->SetParameterName("v_x","v_y","v_z",false);
	translateSiPM->AvailableForStates(G4State_PreInit,G4State_Idle);
...
void SiPMMessenger::SetNewValue(G4UIcommand* command,G4String newValue)
{
	...
	if( command == translateSiPM)
	  {
	    G4ThreeVector translation = translateSiPM->GetNew3VectorValue(newValue);
		mySiPM->SetTranslation(translation);

		G4cout << " The SiPM has now been translated" << G4endl;
